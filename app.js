...
function renderBanner() {
  ...
  if (photos.length > 0) {
    ...
    if (resolved.length > 0) {
      bg = `<div class="banner-slides-wrap" id="banner-slides">
        ${resolved.map((u, i) => `<div class="banner-slide${i === 0 ? ' visible' : ''}" style="background-image:url('${esc(u)}')"></div>`).join('')}
      </div>`;
    } else {
  ...
  if (photos.length > 1) {
    const slideEls = area.querySelectorAll('.banner-slide');
    slideEls.forEach(s => s.classList.remove('visible'));
    let idx = Math.floor(Math.random() * slideEls.length);
    slideEls[idx].classList.add('visible');
    _slideshowTimer = setInterval(() => {
      slideEls[idx].classList.remove('visible');
...
function removeBannerPhoto(idx) {
  data.days[currentDay].banner.photos[idx] = null;
  save();
  renderPhotoGrid();
}

function replaceBannerPhoto(input, slotIdx) {
  const file = input.files[0];
  if (!file) return;
  showUploadStatus('上傳中...');
  uploadToImgBB(file)
    .then(url => { data.days[currentDay].banner.photos[slotIdx] = url; save(); renderPhotoGrid(); })
    .catch(err => alert('上傳失敗：' + err.message))
    .finally(() => showUploadStatus(''));
}
